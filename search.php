...
//$sql = "SELECT * FROM location WHERE loc_e LIKE '%". $keyword . "%' ORDER BY loc_e ASC LIMIT 0, 10";
...
while ($row=  mysqli_fetch_assoc($result)){
    ...
    echo '<li><a href="#" id="searchResult" onclick="geocode('. $id . ','.$lat.','.$lng.','.$qty.',\''.$loc_c.'\',\''.$loc_e.'\')">'.$row['are_c'].' '.$row['loc_c'].'・'.$row['dis_c'].'</a></li>';
}
