...
$qr = "select reg_e, reg_c from region";
...
while ($row = mysqli_fetch_assoc($result_l)) {
    ...
    for ($r = 0; $r < count($regs); $r++) {
        ...
        echo '<a class="icon subMenu" href="#" >' . $reg . '</a>';
        echo '<div class="mp-level">';
        echo '<h2 class="icon">' . $reg . '</h2>';
        ...
        for ($d = 0; $d < count($diss); $d++) {
            if ($diss[$d]['reg_id'] == $r + 1) {
                ...
                echo '<ul>';
                echo '<li class="icon icon-arrow-left-2">';
                echo '<a class="icon subMenu" href="#">' . $dis . '</a>';

                echo '<div class="mp-level">';
                echo '<h2 class="icon">' . $dis . '</h2>';
                echo '<a class="mp-back" href="#">返回</a>';
                ...
                for ($a = 0; $a < count($ares); $a++) {
                    if ($ares[$a]['dis_id'] == $d + 1) {
                        ...
                        echo '<ul>';
                        echo '<li class="icon icon-arrow-left-2">';
                        echo '<a class="icon subMenu" href="#" >' . $are . '</a>';
                        echo '<div class="mp-level">';
                        echo '<h2 class="icon">' . $are . '</h2>';
                        echo '<a class="mp-back" href="#">返回</a>';
...
//                            echo echo '<a href="#" class="subMenu" id="subMenu" onclick="goToLocation(' . $id . ',' . $lat . ',' . $lng . ',' . $qty . ',\'' . $loc_c . '\',\'' . $loc_e . '\')">' . $loc . '</a></li></ul>';
                                echo '<a href="#" class="subMenu" onclick="goLocation(' . $id . ')">' . $loc . '</a></li></ul>';
                            }
                        }
